...
@media (max-width: 600px) {
        ...
        if (!empty($device_specs)) {
            foreach ($device_specs as $key => $spec) {
                ...
                <div class="specification-item">
                    ...
                    <input type="hidden" name="device_specs[<?php echo esc_attr($key); ?>][thumbnail]" value="<?php echo esc_url(!empty($spec['thumbnail']) ? $spec['thumbnail'] : ''); ?>" class="device-thumbnail-url" />
                   
                    <img src="<?php echo esc_url(!empty($spec['thumbnail']) ? $spec['thumbnail'] : ''); ?>" class="device-thumbnail-preview" style="max-width:100px; max-height:100px; display:<?php echo empty($spec['thumbnail']) ? 'none' : 'block'; ?>" />
                    <button type="button" class="upload-thumbnail-button button">رفع الصورة المصغرة</button>
    ...
    jQuery(document).ready(function($) {
        function createMediaUploader(button, type) {
            ...
            mediaUploader.on('select', function() {
                ...
                if (type === 'icon') {
                    ...
                    container.find('.device-thumbnail-url').val(attachment.url);
                    container.find('.device-thumbnail-preview').attr('src', attachment.url).show();
                }
            });
        ...
        $('#add-specification').click(function() {
            ...
            $('#device-specifications-container').append(`
                <div class="specification-item">
                    ...
                    <!-- Thumbnail Upload Field -->
                    <input type="hidden" name="device_specs[` + newIndex + `][thumbnail]" class="device-thumbnail-url" />
                    <img src="" class="device-thumbnail-preview" style="max-width:100px; max-height:100px; display:none;" />
                    <button type="button" class="upload-thumbnail-button button">رفع الصورة المصغرة</button>
        ...
        // Handle image upload for each icon button
        $(document).on('click', '.upload-icon-button', function(e) {
            e.preventDefault();
            var button = $(this);
            var mediaUploader = createMediaUploader(button, 'icon'); // Create uploader for icon
            mediaUploader.open();
        });

        // Handle image upload for each thumbnail button
        $(document).on('click', '.upload-thumbnail-button', function(e) {
            e.preventDefault();
            var button = $(this);
            var mediaUploader = createMediaUploader(button, 'thumbnail'); // Create uploader for thumbnail
            mediaUploader.open();
        });
    });
    </script>
